...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// VMMouse Class Declaration
//

class VMMouse : public IOHIPointing 
{
  OSDeclareDefaultStructors(VMMouse);

private:
  ApplePS2MouseDevice * _device;
  unsigned              _interruptHandlerInstalled:1;
  unsigned              _powerControlHandlerInstalled:1;
  unsigned				_vmmouseAvailable:1;
  unsigned				_useAbsoluteMode:1;
  unsigned				_absoluteModeRequested:1;
...
public:
  ...
  virtual VMMouse * probe(IOService * provider, SInt32 * score);
