...
extern "C"
{
#include "shared/vmmouse_client.h"
}

// =============================================================================
// VMMouse Class Implementation
//

#define super IOHIPointing
OSDefineMetaClassAndStructors(VMMouse, IOHIPointing);

UInt32 VMMouse::deviceType()  { return NX_EVS_DEVICE_TYPE_TABLET; };
UInt32 VMMouse::interfaceID() { return NX_EVS_DEVICE_INTERFACE_BUS_ACE; };

IOItemCount VMMouse::buttonCount() { return _buttonCount; };
IOFixed     VMMouse::resolution()  { return _resolution; };

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

bool VMMouse::init(OSDictionary * properties)
{
  //
  ...
  _buttonCount               = 3;
  _mouseInfoBytes            = static_cast<UInt32>(-1);
  _mouseResetCount			 = 0;
  _vmmouseAvailable			 = false;
  _useAbsoluteMode			 = true;
  _absoluteModeRequested	 = false;

  OSBoolean* noAbsolute = OSDynamicCast(OSBoolean, getProperty("NoAbsolute"));
  if (noAbsolute && noAbsolute->isTrue())
	  _useAbsoluteMode = false;
  SInt16 margin = 0;
  OSNumber* number = OSDynamicCast(OSNumber, getProperty("Margin"));
  if (number) {
    UInt16 umargin = number->unsigned16BitValue();
    if (umargin <= 32767U)
      margin = static_cast<SInt16>(umargin);
  }
  _bounds.minx				= margin;
  _bounds.maxx				= 32767 - margin;
  _bounds.miny				= margin;
  _bounds.maxy				= 32767 - margin;

#if 0
  /*
   * This doesn't work, keeps getting overwritten
   */
  setProperty("Manufacturer", "VMWare Inc.");
  setProperty("VendorID", 0x15ADULL, 32U);
#endif
...
VMMouse * VMMouse::probe(IOService * provider, SInt32 * score)
{ 
...
#if 0
  ApplePS2MouseDevice * device  = OSDynamicCast(ApplePS2MouseDevice, provider);
...
#else
	success = (VMMouseClient_Enable() != FALSE);
	if (success)
...
bool VMMouse::start(IOService * provider)
{ 
  //
  ...
  //
	...
	_device->installInterruptAction(
		this,
		OSMemberFunctionCast(PS2InterruptAction, this, &VMMouse::interruptOccurred));
  ...
  //
	...
	_device->installPowerControlAction(
		this,
		OSMemberFunctionCast(PS2PowerControlAction, this, &VMMouse::setDevicePowerState));
...
void VMMouse::stop(IOService * provider)
{
  //
...
void VMMouse::resetMouse()
{
  ...
  if (_mouseInfoBytes == static_cast<UInt32>(-1))
  {
    _mouseInfoBytes = getMouseInformation();
    switch (_mouseInfoBytes & 0x00FF00U)
    {
  ...
  {
#ifdef VMX86_DEVEL
	  if (type == kMouseTypeIntellimouseExplorer)
		  IOLog("%s: Detected IntellimouseExplorer\n", getName());
	  else
		  IOLog("%s: Detected Intellimouse\n", getName());
#endif
...
void VMMouse::scheduleMouseReset()
{
  //
...
void VMMouse::interruptOccurred(UInt8 data)      // PS2InterruptAction
{
  //
...
void VMMouse::dispatchRelativePointerEventWithPacket(UInt8 * packet,
                                                           UInt32  packetSize)
{
	UInt32       buttons = 0;
	SInt32       dx;
	...
	if (_vmmouseAvailable) {
		...
		IOGPoint pt;
		int numPackets;
		bool _isCurrRelative;

		if (_useAbsoluteMode && !_absoluteModeRequested) {
			VMMouseClient_RequestAbsolute();
			_absoluteModeRequested = true;
		}
		while ((numPackets = VMMouseClient_GetInput(&vmmouseInput)))
		{
			...
			dx = vmmouseInput.X;
			dy = vmmouseInput.Y;
			dz = static_cast<SInt8>(vmmouseInput.Z);
#if 0 && defined VMX86_DEVEL
			IOLog("%s: x == %d, y == %d, z == %d, buttons == %u\n", getName(), dx, dy, static_cast<int>(dz), buttons);
#endif
			_isCurrRelative = ((vmmouseInput.Flags & VMMOUSE_MOVE_RELATIVE) != 0);
			clock_get_uptime(reinterpret_cast<uint64_t*>(&now));
			if (_isCurrRelative)
				dispatchRelativePointerEvent(dx, -dy, buttons, now);
			else {
				pt.x = static_cast<SInt16>(dx/2);
				pt.y = static_cast<SInt16>(dy/2);
...
void VMMouse::setMouseEnable(bool enable)
{
  //
  ...
  if (enable)
    _vmmouseAvailable = (VMMouseClient_Enable() != FALSE);
  else {
    VMMouseClient_Disable();
    _vmmouseAvailable = false;
  }
...
void VMMouse::setMouseSampleRate(UInt8 sampleRate)
{
  //
...
void VMMouse::setMouseResolution(UInt8 resolution)
{
  //
...
PS2MouseId VMMouse::setIntellimouseMode()
{
  //
...
UInt32 VMMouse::getMouseInformation()
{
  //
  ...
  PS2Request * request     = _device->allocateRequest();
  UInt32       returnValue = static_cast<UInt32>(-1);
...
UInt8 VMMouse::getMouseID()
{
  ...
  PS2Request * request     = _device->allocateRequest();
  UInt8        returnValue = static_cast<UInt8>(-1);
...
void VMMouse::setCommandByte(UInt8 setBits, UInt8 clearBits)
{
  //
...
void VMMouse::setDevicePowerState( UInt32 whatToDo )
{
    switch ( whatToDo )
    {
